...
const flaskPath = "http://localhost:8080/";
...
export default class ControlPanel extends React.Component {
    ...
    constructor(props) {
        ...
        this.props.constraintHandler(this.state);
    ...
    getCounties() {
        ...
        if(this.state.propVal[1].propValue === '1') {
            ...
            Request.get(flaskPath+'/cdc/rates/'+state, (error, response, body) => {
                if(error) {
            ...
            Request.get(flaskPath+'/gaz/'+state, (error, response, body) => {
                if(error) {
                    return console.log("WHAT ERROR?");
                }
                else {
    ...
    getOptions(optionType) {
        if ( optionType === 'state' ) {
            var Request = require('request');
            Request.get(flaskPath+'/st', (error, response, body) => {
                if(error) {
                    return console.log("WHAT ERROR?");
                }
                else {
